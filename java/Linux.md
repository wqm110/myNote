...
cat file1 file2 >file  几个文件合并成一个
-b 对非空输出行号
-n 输出所有行号
...
chmod a+x test.txt            #增加文件 t.log 所有用户可执行权限
chmod 755 test.txt
...
-exec： find命令对匹配的文件执行该参数所给出的shell命令。相应命令的形式为'command' {  } \;，注意{   }和\；之间的空格。
 参数后面跟的是 command 命令，它的终止是以 ; 为结束标志的，所以这句命令后面的分号是不可缺少的，考虑到各个系统中分号会有不同的意义，所以前面加反斜杠。{} 花括号代表前面find查找出来的文件名。
...
#（其中.表示一个字符，*表示任务多个；.*表示任意多个字符）查找 /var 目录下，以 reason 结尾的文件
locate -r '^/var.*reason$'
```
...
![vim 工作模式](https://tva1.sinaimg.cn/large/008eGmZEgy1gmjpa2y30tj30lv0erabh.jpg)



![vi-vim-cheat-sheet-sch](https://tva1.sinaimg.cn/large/008eGmZEgy1gmjp8vvky4g30sg0k40w3.gif)
