...
![image-20210104043323093](https://tva1.sinaimg.cn/large/0081Kckwgy1gmb64toqeuj31490hddl0.jpg)

![image-20210104043357207](https://tva1.sinaimg.cn/large/0081Kckwgy1gmb65epwdxj30ue0kkwr2.jpg)
...
### CountDownLatch 闭锁 减锁

![image-20210104044528354](https://tva1.sinaimg.cn/large/0081Kckwgy1gmb6henjidj30w20k0wqy.jpg)
 ...
 * @ProjectName: JUC
 * @Package: atomic.lock
 * @ClassName: CountDownLan
 * @Author: WQM
 * @Description: 倒计时
 * @Date: 2021/1/4 9:40 上午
 * @Version: 1.0
 */
public class CountDownLan {
    public static void main(String[] args) throws InterruptedException {
        FireLanch lanch = new FireLanch();
        for (int i = 0; i < 15; i++) {
...
class FireLanch {
    ...
    CountDownLatch killDownLanthc = new CountDownLatch(7);

    public void killsix(int id) {
        EnmuDemo enmu = EnmuDemo.getEnmu(id);
        if (null != enmu) {
            System.out.println("灭  -->" + enmu.getName()); 
        }
        killDownLanthc.countDown();
    }

    public void union() throws InterruptedException {
        killDownLanthc.await();
        System.out.println("公元前220 年秦统一六国。。");
    }
    ...
    public void finish() {
        ...
        System.out.println("countDownLatch = " + countDownLatch + "\t finish");
    }
}
```
 ...
 * @Author: WQM
 * @Description: 枚举
 * @Date: 2021/1/4 10:02 上午
 * @Version: 1.0
 */
public enum EnmuDemo {
    QI(1, "齐国"), CHU(2, "楚国"), YAN(3, "燕国"),
    HAN(4, "韩国"), ZHAO(5, "赵"), WEI(6, "魏"),
    QIN(7, "秦");
    ...
    EnmuDemo(int conde, String name) {
        this.code = conde;
        this.name = name;
    }

    public static EnmuDemo getEnmu(int id) {
        EnmuDemo[] values = EnmuDemo.values();
        for (EnmuDemo e : values
        ) {
            if (e.getCode() == id) {
                return e;
            }
 ...
 * @Author: WQM
 * @Description:
 * @Date: 2021/1/4 10:43 上午
 * @Version: 1.0
 */
public class CyclicBarrierDemo {
    public static void main(String[] args) {
        DrongBole drongBole = new DrongBole();
        for (int i = 1; i < 8; i++) {
            new Thread(() -> {
                drongBole.colicDrongbole();
...
class DrongBole{
        CyclicBarrier cyclicBarrier = new CyclicBarrier(7,()->{
            System.out.println("成功集齐七科龙珠。。");
        });
    public void colicDrongbole(){
        try {
            System.out.print("找到一颗。。");
            cyclicBarrier.await();
        } catch (InterruptedException e) {
            e.printStackTrace();
...
class Strop {
    ...
    public void stop(int id) throws InterruptedException {
        //占到车位
        semaphore.acquire();
        System.out.println(id + "抢到车位.."+"还剩："+semaphore.getQueueLength());
        TimeUnit.SECONDS.sleep(5);
        semaphore.release();
        System.out.println(id + "开走了。。"+"还剩："+semaphore.getQueueLength());
    }
}
```
 ...
 * @Author: WQM
 * @Description:
 * @Date: 2021/1/4 5:11 上午
 * @Version: 1.0
 */
public class LockTest {
    public static void main(String[] args) { 
        Ticket ticket = new Ticket();
        FutureTask<Integer> ticketFutureTask = new FutureTask<>(ticket);
         FutureTask<Integer> ticketFutureTask2 = new FutureTask<>(ticket); 
       
        new Thread(ticketFutureTask2).start();
        new Thread(ticketFutureTask).start();
        try {
            Integer result = ticketFutureTask.get();
 ...
 * @ProjectName: JUC
 * @Package: atomic.lock
 * @ClassName: ReentrenLoc
 * @Author: WQM
 * @Description: 重入锁
 * @Date: 2021/1/4 7:40 上午
 * @Version: 1.0
 */
public class ReentrenLoc {
    public static void main(String[] args) {
        for (int i = 1; i < 10; i++) {
            new Thread(() -> {
                new ReLock().openPhone();
            }, String.valueOf(i)).start();
        }
    }
}

class ReLock implements Runnable {
    Lock lock = new ReentrantLock();
 ...
 * @Author: WQM
 * @Description:
 * @Date: 2021/1/4 6:35 上午
 * @Version: 1.0
 */
public class SelfLoack {
    public static void main(String[] args) throws InterruptedException, ExecutionException {

        MyLock myLock = new MyLock();
        FutureTask<Integer> lockFutureTask = new FutureTask<>(myLock);
        new Thread(lockFutureTask).start();
        ...
        new Thread(lockFutureTask).start();
        new Thread(lockFutureTask).start();
        Integer integer = lockFutureTask.get();
        System.out.println("integer = " + integer);
    }
} 
class MyLock implements Callable<Integer> {
    volatile int b = 100;


    public boolean checkNum(int v1) {
        if (v1 > b) {
            return false;
        } else {
            b--;
    ...
    public Integer call() throws Exception {
        ...
        do {
            bl = checkNum(var);
           System.out.println("等待中 var: " + var + "\t b" + b);
            try {
                Thread.sleep(30);
            } catch (InterruptedException e) {
                e.printStackTrace();
...
public class ReadWriteLockDemo {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < 5; i++) {
            int finalI = i;
            new Thread(() -> {
                myRWLock.writeLock(String.valueOf(finalI), String.valueOf(finalI));
            }, String.valueOf(i)).start();
        }
        for (int i = 1; i < 5; i++) {
            int finalI = i;
            new Thread(() -> {
                myRWLock.readLock(String.valueOf(finalI));
            }, String.valueOf(i)).start();
        }
    }
}
...
class MyRWLock {
    ...
    MyRWLock(String k, String v) {
        this.k = k;
        this.v = v;
    } 
    public Lock readLock(String k) {
        lock.readLock().lock();
        try {
            System.out.println("正在读取数据" + k + Thread.currentThread().getName());
            String v = map.get(k);
            System.out.println("正在读k:v = " + k + "\t:" + v + Thread.currentThread().getName());
            try {
                Thread.sleep(300);
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    public Lock writeLock(String k,String v) {
        lock.writeLock().lock();
        try {
            System.out.println("正在写入" + v + Thread.currentThread().getName());
            map.put(k, v);
            try {
                Thread.sleep(300);
            } catch (InterruptedException e) {
            ...
            System.out.println("写入完成 k:v = " + k + "\t:" + v + Thread.currentThread().getName());
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
...
![image-20210104112617260](https://tva1.sinaimg.cn/large/0081Kckwgy1gmbi2gj0m6j318c0eo45p.jpg)

![image-20210104120339504](https://tva1.sinaimg.cn/large/0081Kckwgy1gmbj5bwp1lj317o09g438.jpg)

#### ArrayBlockingQueue

##### 异常组 add romove element
 ...
 * @ProjectName: JUC
 * @Package: queen
 * @ClassName: BlockQueenDemo
 * @Author: WQM
 * @Description: 队列
 * @Date: 2021/1/4 11:31 上午
 * @Version: 1.0
 */
public class BlockQueenDemo {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < 20; i++) {
            new Thread(() -> {
//                factory.add(bread.getBread());//exception
                try {
                       factory.offer(bread);                //boolean
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }, String.valueOf(i)).start();
        ...
        for (int i = 1; i < 20; i++) {
            new Thread(() -> {
//                factory.remove();           //exception
                try {
                    factory.poll();   //boolean
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }, String.valueOf(i)).start();
...
class Factory {
    //面包柜15个
    BlockingQueue<Bread> queue = new ArrayBlockingQueue<Bread>(15);

    public void add(Bread bread) {
        System.out.print("放入：1个 \t");
        queue.add(bread);                                           //Exception group
        System.out.println("柜子里剩余：" + (15 - queue.size()));
    }

    public Bread remove() {
        System.out.print("取走：1个 \t");
        Bread removed = queue.remove();                             //Exception group
        System.out.println("柜子里剩余：" + (15 - queue.size()));
        return removed;
    }

    public void offer(Bread bread) throws InterruptedException {
        System.out.print("放入：1个 \t");
        queue.offer(bread,4L, TimeUnit.SECONDS);             				//TIME group
        System.out.println("柜子里剩余：" + (15 - queue.size()));
    }
    ...
    public Bread poll() throws InterruptedException {
        ...
        Bread removed = queue.poll(2,TimeUnit.SECONDS);        			//TIME group
        System.out.println("柜子里剩余：" + (15 - queue.size()));
        return removed;
    }
    ...
    public Bread take() throws InterruptedException {
        System.out.print("取走：1个 \t");
        Bread removed = queue.take();                                   //阻塞
        System.out.println("柜子里剩余：" + (15 - queue.size()));
        return removed;
    }
}
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            new Thread(() -> {
                try {
                    TimeUnit.SECONDS.sleep(5);
                    System.out.println("take-->"+ finalI1);
                    queue.take();
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
...
![image-20210104122542734](https://tva1.sinaimg.cn/large/0081Kckwgy1gmbjsaho84j31500h2dhk.jpg)

![image-20210104131438306](https://tva1.sinaimg.cn/large/0081Kckwgy1gmbl7719c4j31c70ktarg.jpg)

![image-20210104133331438](https://tva1.sinaimg.cn/large/0081Kckwgy1gmblqucrs4j30mv05qdhx.jpg)
 ...
 * @Author: WQM
 * @Description: 练习题
 * @Date: 2021/1/4 1:22 下午
 * @Version: 1.0
 */
//

/**
 * 1 AA*5 --> 2 BB*5 --> 3 CC*5
 * times 10
 */
public class Print5 {
...
class Printer {
    ...
    private volatile String target = "AA";
 ...
 * @Author: WQM
 * @Description: 生产者消费者
 * @Date: 2021/1/4 3:05 下午
 * @Version: 1.0
 */
public class ProductConsumer {
    public static void main(String[] args) {
        MyResource myResource = new MyResource(new ArrayBlockingQueue(18));
        new Thread(() -> {
            //方法调用
            try {
                myResource.product();
...
class MyResource {
    ...
    MyResource(BlockingQueue blockingQueue) {
        ...
        System.out.println("当前队列类型为 ： " + blockingQueue.getClass().getName());
    }
    ...
    public void product() throws InterruptedException {
        ...
        System.out.println(Thread.currentThread().getName() + "\t Flag=false 老板叫停了线程");
    }
    ...
    public void consume() {
        ...
        while (FLAG) {
            try {
                poll = blockingQueue.poll(3L, TimeUnit.SECONDS);
                if (poll != null) {
...
![image-20210105081100108](https://tva1.sinaimg.cn/large/0081Kckwgy1gmci1kfpefj30z50f40xu.jpg)
